...
import { getDailyReward, setDailyReward, addCoins, getXP, upsertXP, incrementStat } from "./db";
import { checkBadges, BADGES } from "./badgeSystem";
...
export async function postDailyMenuIfNeeded(guild: Guild, botId: string): Promise<void> {
  const ch = guild.channels.cache.find(
    (c) =>
      ...
      (c.name.toLowerCase().includes("daily") ||
        c.name.toLowerCase().includes("quotidien") ||
        c.name.toLowerCase().includes("recompense") ||
        c.name.toLowerCase().includes("récompense"))
  ...
  try {
    ...
    const alreadyPosted = recent.some(
      (m) => m.author.id === botId && m.embeds[0]?.title?.includes("Récompense quotidienne")
    );
    if (alreadyPosted) return;
    await ch.send({ embeds: [buildDailyEmbed()], components: buildDailyComponents() });
    logger.info(`✅ Menu daily posté dans #${ch.name}`);
  } catch (err) {
    logger.warn({ err }, `Impossible de poster le menu daily dans #${ch.name}`);
  }
}
...
export async function handleDailyClaim(btn: ButtonInteraction): Promise<void> {
  ...
  if (coins > 0) {
    newBalance = await addCoins(guildId, userId, coins);
    await incrementStat(guildId, userId, "coins_earned_total", coins);
  }
  if (xp > 0) {
  ...
  await checkBadges(member, stats, newStreak).catch(() => {});

  const streakBonus = mult > 1 ? ` *(x${mult.toFixed(1)} streak 🔥)*` : "";
  const streakBadge = BADGES.find(b => b.stat === "streak" && newStreak === b.threshold);
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          ...
          ...(streakBadge ? [{ name: "🏅 Badge débloqué !", value: `**${streakBadge.name}**`, inline: false }] : []),
        )
    ...
    embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: now < next ? "⏳ Prochain claim" : "✅ Disponible !", value: now < next ? `<t:${Math.floor(next / 1000)}:R>` : "Maintenant !", inline: true },
        )
