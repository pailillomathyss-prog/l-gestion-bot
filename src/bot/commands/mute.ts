...
export async function muteCommand(message: Message, args: string[]) {
  ...
  await target.send({ embeds: [new EmbedBuilder().setColor(0xffa500).setTitle("🔇 Tu as été muté")
    .addFields({ name: "Serveur", value: message.guild!.name }, { name: "Durée", value: formatted },
      { name: "Raison", value: reason }, { name: "Modérateur", value: message.author.tag }).setTimestamp()] })
    .catch(() => {});

  await message.channel.send({ embeds: [new EmbedBuilder().setColor(0xffa500).setTitle("🔇 Membre muté")
...
export async function demuteCommand(message: Message, args: string[]) {
  ...
  await message.channel.send({ embeds: [new EmbedBuilder().setColor(0x57f287).setTitle("🔊 Membre démuté")
    .addFields({ name: "Utilisateur", value: `${target.user.tag} (${target.id})` },
      { name: "Raison", value: reason }, { name: "Modérateur", value: message.author.tag })
    .setThumbnail(target.user.displayAvatarURL()).setTimestamp()] });
}
