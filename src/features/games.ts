...
const RARITY_COLOR: Record<string,number> = { Commun:0x95a5a6,"Peu Commun":0x2ecc71,Rare:0x9b59b6,Épique:0x00bcd4,Légendaire:0xffd700,Mythique:0xe74c3c };

function pickGacha() { let r=Math.random()*GACHA_TOTAL; for(const x of GACHA_ROLES){r-=x.weight;if(r<=0)return x;} return GACHA_ROLES[0]; }

const duels = new Map<string,{challengerId:string;bet:number}>();
const GAME_CAT = "🎮 Parties en Cours";

// ── Temp channel helpers ──────────────────────────────────────────────────────
async function getOrCreateCat(guild:Guild): Promise<CategoryChannel> {
  let cat = guild.channels.cache.find(c=>c.type===ChannelType.GuildCategory&&c.name===GAME_CAT) as CategoryChannel|undefined;
  if (!cat) cat = await guild.channels.create({ name:GAME_CAT, type:ChannelType.GuildCategory, permissionOverwrites:[{id:guild.roles.everyone,deny:[PermissionFlagsBits.ViewChannel]}], reason:"MAI•GESTION" });
  return cat;
}

async function openGameChannel(guild:Guild, game:string, ...userIds:string[]): Promise<TextChannel|null> {
  try {
    const cat  = await getOrCreateCat(guild);
    const m    = guild.members.cache.get(userIds[0]!);
    const slug = `${game}-${m?.user.username??userIds[0]!}`.toLowerCase().replace(/[^a-z0-9-]/g,"").slice(0,90);
...
export async function postGamePanelIfNeeded(guild:Guild, botId:string) {
  const ch = guild.channels.cache.find(c=>c.type===ChannelType.GuildText&&(c.name.includes("jeux")||c.name.includes("👾"))) as TextChannel|undefined;
  if (!ch) return;
...
export async function postGameRulesIfNeeded(guild:Guild, botId:string) {
  ...
  await ch.send({embeds:[new EmbedBuilder().setColor(0x2f3136).setTitle("📜 Règles des Jeux — MAI•GESTION")
    .setDescription("Chaque partie crée un **salon privé** qui se ferme à la fin.")
    .addFields(
      {name:"💰 Pièces",     value:"• Messages : 10–20🪙/min\n• Vocal : 15🪙/5min\n• `!daily` : 50–300🪙"},
      {name:"🎮 Jeux",       value:"🪙 Coin Flip (50/50) | 🎰 Slots (×2→×20) | 🃏 Blackjack (×2) | 🎲 Duel 1v1 | 🎁 Gacha (150🪙)"},
      {name:"🎁 Raretés",    value:"🎀🌿 Commun 55% | ⚡🌸 Peu Commun 25% | 💜🔥 Rare 12% | 💎🌟 Épique 6% | 👑🌌 Légendaire 1.8% | ⚜️ Mythique 0.2%"},
      {name:"⚠️ Important", value:"• Mise minimum : 10🪙\n• Impossible de miser plus que son solde\n• Rôles gacha créés automatiquement"},
...
async function doFlip(btn:ButtonInteraction, bet:number) {
  ...
  if(data.coins<bet){await btn.editReply({content:`❌ Tu as **${data.coins}🪙** mais tu mises **${bet}🪙**.`});return;}
  const win   = Math.random()<0.5;
  const delta = win?bet:-bet;
  await saveUser(btn.guild.id,btn.user.id,{...data,coins:data.coins+delta});
  const embed = new EmbedBuilder().setColor(win?0x00cc66:0xff4444)
  ...
  const gameCh = await openGameChannel(btn.guild,"coinflip",btn.user.id);
  if(gameCh){
    ...
    await btn.editReply({content:`🎮 Ta partie → <#${gameCh.id}> (fermé dans 45s)`});
  } else await btn.editReply({embeds:[embed]});
}
...
async function doSlots(btn:ButtonInteraction, bet:number) {
  ...
  await btn.deferReply({ephemeral:true});
  const data = await getUser(btn.guild.id,btn.user.id);
  if(data.coins<bet){await btn.editReply({content:`❌ Tu as **${data.coins}🪙**.`});return;}
  ...
  else if(r[0]===r[1]||r[1]===r[2]||r[0]===r[2]){delta=Math.floor(bet*1.5);txt=`✨ **2 identiques !** →**+${delta}🪙**`;}
  else{delta=-bet;txt=`💸 **Rien…** → **-${bet}🪙**`;}
  await saveUser(btn.guild.id,btn.user.id,{...data,coins:data.coins+delta});
  const embed=new EmbedBuilder().setColor(delta>0?0x00cc66:0xff4444).setTitle("🎰 Machine à sous")
    .setDescription(`**${r.join(" | ")}**\n\n${txt}\n💰 Solde : **${(data.coins+delta).toLocaleString("fr-FR")}🪙**`).setFooter({text:"MAI•GESTION"}).setTimestamp();
  const gameCh=await openGameChannel(btn.guild,"slots",btn.user.id);
  if(gameCh){await gameCh.send({content:`<@${btn.user.id}>`,embeds:[embed.setFooter({text:"MAI•GESTION • Fermé dans 45s"}).setTimestamp()]}).catch(()=>{});closeAfter(gameCh,45_000);await btn.editReply({content:`🎮 Ta partie → <#${gameCh.id}>`});}
  else await btn.editReply({embeds:[embed]});
}
...
async function doBJ(btn:ButtonInteraction, bet:number) {
  ...
  const pT=tot(p);
  let delta=0,result="";
  if(pT===21){delta=Math.floor(bet*1.5);result=`🎉 **Blackjack !** ×1.5 → **+${delta}🪙**`;}
  else{let dT=tot(d);while(dT<17)d.push(card()),dT=tot(d);if(pT>21){delta=-bet;result=`💥 Bust (${pT}) → **-${bet}🪙**`;}else if(dT>21||pT>dT){delta=bet;result=`✅ Victoire ! ${pT} vs ${dT} → **+${bet}🪙**`;}else if(pT===dT){delta=0;result=`🤝 Égalité ${pT} — Remboursé`;}else{delta=-bet;result=`❌ Défaite ${pT} vs ${dT} → **-${bet}🪙**`;}}
  await saveUser(btn.guild.id,btn.user.id,{...data,coins:data.coins+delta});
  const embed=new EmbedBuilder().setColor(delta>0?0x00cc66:delta===0?0xffd700:0xff4444).setTitle("🃏 Blackjack")
    .addFields({name:"🧑 Toi",value:`**${p.join("+")} = ${pT}**`,inline:true},{name:"🏦 Croupier",value:`**${d.join("+")} = ${tot(d)}**`,inline:true})
    .setDescription(`${result}\n💰 Solde : **${(data.coins+delta).toLocaleString("fr-FR")}🪙**`).setFooter({text:"MAI•GESTION"}).setTimestamp();
  const gameCh=await openGameChannel(btn.guild,"blackjack",btn.user.id);
  if(gameCh){await gameCh.send({content:`<@${btn.user.id}>`,embeds:[embed.setFooter({text:"MAI•GESTION • Fermé dans 60s"}).setTimestamp()]}).catch(()=>{});closeAfter(gameCh,60_000);await btn.editReply({content:`🎮 Ta partie → <#${gameCh.id}>`});}
  else await btn.editReply({embeds:[embed]});
}
...
async function doDuel(btn:ButtonInteraction, bet:number) {
  if(!btn.guild) return;
  const data=await getUser(btn.guild.id,btn.user.id);
  if(data.coins<bet){await btn.reply({content:`❌ Tu as **${data.coins}🪙**.`,ephemeral:true});return;}
  await btn.reply({
    ...
    components:[new ActionRowBuilder<UserSelectMenuBuilder>().addComponents(new UserSelectMenuBuilder().setCustomId(`g_duel_pick:${bet}`).setPlaceholder("Choisis ton adversaire…").setMinValues(1).setMaxValues(1))],
    ephemeral:true,
  });
}
...
async function doGacha(btn:ButtonInteraction) {
  ...
  const embed=new EmbedBuilder().setColor(RARITY_COLOR[picked.rarity]??0x9b59b6).setTitle(`${picked.emoji} ${picked.rarity} !`)
    .setDescription(has?`Tu as tiré **${picked.name}**… déjà obtenu ! 😅\n💰 Solde : **${(data.coins-GACHA_PRICE).toLocaleString("fr-FR")}🪙**`:`🎉 Rôle **${picked.name}** obtenu !\n💰 Solde : **${(data.coins-GACHA_PRICE).toLocaleString("fr-FR")}🪙**`)
    .addFields({name:"📊 Chances",value:"🎀🌿 55% | ⚡🌸 25% | 💜🔥 12% | 💎🌟 6% | 👑🌌 1.8% | ⚜️ 0.2%"}).setFooter({text:"MAI•GESTION"}).setTimestamp();
  const gameCh=await openGameChannel(btn.guild,"gacha",btn.user.id);
  if(gameCh){await gameCh.send({content:`<@${btn.user.id}>`,embeds:[embed.setFooter({text:"MAI•GESTION • Fermé dans 60s"}).setTimestamp()]}).catch(()=>{});closeAfter(gameCh,60_000);await btn.editReply({content:`🎮 Ton tirage → <#${gameCh.id}>`});}
  else await btn.editReply({embeds:[embed]});
}
...
export async function handleGameButton(btn:ButtonInteraction) {
  ...
  if(id.startsWith("g_duel_accept:")) {
    ...
    await Promise.all([saveUser(btn.guild.id,wId,{...wData,coins:wData.coins+bet}),saveUser(btn.guild.id,lId,{...lData,coins:Math.max(0,lData.coins-bet)})]);
    const resultEmbed=new EmbedBuilder().setColor(0xffd700).setTitle("🎲 Duel terminé !").setDescription(`🏆 <@${wId}> remporte **${bet*2}🪙** contre <@${lId}> !`).setFooter({text:"MAI•GESTION"}).setTimestamp();
    const gameCh=await openGameChannel(btn.guild,"duel",duel.challengerId,tid!);
    if(gameCh){await gameCh.send({embeds:[resultEmbed.setFooter({text:"MAI•GESTION • Salon fermé dans 60s"}).setTimestamp()]}).catch(()=>{});closeAfter(gameCh,60_000);await btn.update({embeds:[resultEmbed],components:[]});}
    else await btn.update({embeds:[resultEmbed],components:[]});
    return;
  }
  if(id.startsWith("g_duel_refuse:")) {
    ...
    await btn.update({embeds:[new EmbedBuilder().setColor(0x888888).setDescription(`❌ **${btn.user.displayName}** a refusé.`).setFooter({text:"MAI•GESTION"}).setTimestamp()],components:[]});
  }
}
