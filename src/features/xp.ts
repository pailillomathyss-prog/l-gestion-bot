...
function topMilestone(level: number) { return [...MILESTONES].reverse().find(m => level >= m.level) ?? null; }

// Rôles créés AU FUR ET À MESURE (seulement quand un joueur atteint le level)
async function applyLevelRole(member: GuildMember, level: number) {
  const guild = member.guild;
  const ms = topMilestone(level);
  if (!ms) return;
  // Créer le rôle s'il n'existe pas encore
  let role = guild.roles.cache.find(r => r.name === ms.name);
  if (!role) role = await guild.roles.create({ name: ms.name, color: ms.color, permissions: [], reason: "MAI•GESTION level up" }).catch(() => undefined);
  if (!role) return;
  // Retirer les rôles de niveau inférieurs
...
async function announceLevel(member: GuildMember, level: number) {
  ...
  await ch.send({ embeds: [new EmbedBuilder()
    .setColor(ms?.color ?? 0x9b59b6).setTitle("🎉 Montée de niveau !")
    .setDescription(`Félicitations <@${member.id}> ! Tu atteins le **niveau ${level}** !${ms ? `\n🎁 Rôle obtenu : **${ms.name}**` : ""}`)
    .setThumbnail(member.user.displayAvatarURL()).setFooter({ text: "MAI•GESTION" }).setTimestamp()
  ] }).catch(() => {});
}
...
export async function handleMessageXP(member: GuildMember) {
  ...
  if (now - data.lastMsgTs < MSG_CD) return;
  ...
  await saveUser(member.guild.id, member.id, { ...data, xp: newXP, level: newLvl, coins: data.coins + cGain, lastMsgTs: now });
  if (lvlUp) { await applyLevelRole(member, newLvl).catch(() => {}); await announceLevel(member, newLvl).catch(() => {}); }
}
...
export async function tickVoiceXP(guild: Guild) {
  for (const [, ch] of guild.channels.cache) {
    ...
    for (const [, member] of (ch as import("discord.js").VoiceChannel).members) {
      ...
      const newLvl = xpToLevel(newXP);
      const lvlUp  = newLvl > data.level;
      await saveUser(guild.id, member.id, { ...data, xp: newXP, level: newLvl, coins: data.coins + C_VOICE });
      if (lvlUp) { await applyLevelRole(member, newLvl).catch(() => {}); await announceLevel(member, newLvl).catch(() => {}); }
    }
  }
}
...
export async function rankCommand(message: Message) {
  ...
  await message.reply({ embeds: [new EmbedBuilder()
    .setColor(ms?.color ?? 0x9b59b6).setTitle(`📊 Profil de ${target.displayName}`)
